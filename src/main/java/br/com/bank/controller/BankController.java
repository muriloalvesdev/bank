...
public class BankController {
    ...
    public ResponseEntity<Bank> debitAuthorization(
            @RequestBody BankDataTransferObject bankDTO) {
        bankService.checkIfThereIsALimitOnTheCardAndDebitTheAvailableAmount(
                bankDTO);
    ...
    @PostMapping("deposit")
    public ResponseEntity<Bank> deposit(
            @RequestBody BankDataTransferObjectDeposit bankDTODeposit) {
        return ResponseEntity
                .ok(bankService.depositValueValueInAccount(bankDTODeposit));
    }
}
